...
class SessionCipher {
    ...
    async encrypt(data) {
        ...
        return await this.queueJob(async () => {
            ...
            // Jika tidak ada record, buat session baru terlebih dahulu
            if (!record) {
                throw new errors.SessionError("No sessions available. Please establish a session first.");
            }
            
            const session = record.getOpenSession();
            if (!session) {
                throw new errors.SessionError("No open session available. Please establish a session first.");
            }
    ...
    // Metode baru untuk membantu membuat session jika belum ada
    async ensureSession() {
        return await this.queueJob(async () => {
            let record = await this.getRecord();
            if (!record) {
    ...
    // Metode untuk mendapatkan informasi session (debugging)
